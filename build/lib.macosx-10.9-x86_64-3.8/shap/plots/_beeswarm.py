...
def reveal_distribution(shap_values, feature_values, feature_name):
    average_shap = np.mean(shap_values)
    average_feature = np.mean(feature_values)

    np_shaps = np.array(shap_values)
    np_values = np.array(feature_values)

    shap_indices_larger = np.argwhere(average_shap >= average_shap).flatten()
    shap_indices_smaller = np.argwhere(average_shap < average_shap).flatten()

    if len(shap_indices_larger) > len(shap_indices_smaller):
        corresponding_feature_values = np_values[shap_indices_larger].flatten()
        corresponding_larger = np.argwhere(corresponding_feature_values >= average_feature).flatten()
        corresponding_smaller = np.argwhere(corresponding_feature_values < average_feature).flatten()
        if len(corresponding_larger) > len(corresponding_smaller):
            print('the larger this feature, the higher the label')
        else:
            print('the smaller this feature, the higher the label')
    else:
        corresponding_feature_values = np_values[shap_indices_smaller].flatten()
        corresponding_larger = np.argwhere(corresponding_feature_values >= average_feature).flatten()
        corresponding_smaller = np.argwhere(corresponding_feature_values < average_feature).flatten()
        if len(corresponding_larger) > len(corresponding_smaller):
            print('the larger this feature, the lower the label')
        else:
            print('the smaller this feature, the lower the label')
    ...
    if plot_type == "dot":
        ...
        if features is not None:
            ...
            for pos, i in enumerate(feature_order):
                ...
                shap_feature_values.append({np.mean(shaps) : np.mean(values)})
